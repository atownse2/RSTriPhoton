...
mc_subsets = {'GJets': ['GJets_HT-40To100', 'GJets_HT-100To200', 'GJets_HT-200To400', 'GJets_HT-400To600', 'GJets_HT-600ToInf'],}
...
#For MC request
BKK_MASS_MC_REQUEST = [ 200, 250, 500, 750, 1000, 1500, 2000, 2500, 3000]
MOE_MC_REQUEST = [0.005, 0.0075, 0.01, 0.025, 0.04]

# M_PHI / E_PHI = M_PHI / (M_BKK/2) = MOE ---> M_PHI = MOE * (M_BKK/2)
mass_grid_mc_request = [ (M_BKK, MOE*(M_BKK/2)) for M_BKK in BKK_MASS_MC_REQUEST for MOE in MOE_MC_REQUEST]
...
def get_signal_cross_section(filetag):
    xs_file = f'{top_dir}/analysis/metadata/json/signal_xs.json'
...
def load_xsdb_info(dataset):
    ...
    xsdb_path = f'{top_dir}/analysis/metadata/XSDB/'
...
#json io
def load_mc_info(dType='signal'):
    '''Loads the MC info from the json file'''
    json_path = f'{script_dir}/json/{dType}_info.json'
...
def save_mc_info(info, dType='signal'):
    '''Saves the MC info to the json file'''
    json_path = f'{script_dir}/json/{dType}_info.json'
...
#Signal functions
def get_signal_filetag(signal_point=None, mc_request=False):
...
def get_all_signal_filetags():
    '''Returns a list of all signal filetags in mass grid'''
    return [get_signal_filetag(point) for point in mass_grid]
...
#I/O functions
def get_all_mc_filetags(backgrounds):
    '''Returns a list of all mc filetags'''
    filetags=[]
    for dataset in backgrounds:
        filetags += mc_subsets[dataset]
    return filetags
